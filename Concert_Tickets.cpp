...
auto findLargestSmallerOrEqual(multiset<ll>& mset, int k) {
    auto it = mset.lower_bound(k);
    if (it == mset.begin()) {
        return (*it == k) ? it : mset.end();
    } 
    else {
        auto prevIt = std::prev(it);
        if (*it == k and it != mset.end()) return it;
        return (*prevIt < k) ? prevIt : it;
    }
}
...
void solve(){
    ...
    fl (i, m){
        int ans = -1;
        if (findLargestSmallerOrEqual(ms, p[i]) != ms.end()){
            ans = *findLargestSmallerOrEqual(ms, p[i]);
            ms.erase(findLargestSmallerOrEqual(ms, p[i]));
        }
        cout << ans << en;
    }
