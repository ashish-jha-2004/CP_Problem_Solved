...
#define fn(i, j, k) for(int i{j}; i<k; fn)
#define no cout << "NO\n"
#define yes cout << "YES\n"
...
template<typename T>
ll sum_vector(vector<T> v) { ll n = v.size(); ll s = 0; fl(i, n) s += v[i]; return s; }
template <typename T>
...
ll mod_inverse(ll i) { if (i == 1) return 1; return (mod - ((mod / i) * mod_inverse(mod % i)) % mod) % mod; }
ll mod_multiply(ll a, ll b) { a = a % mod; b = b % mod; return (((a * b) % mod) + mod) % mod; }
ll mod_add(ll a, ll b) { a = a % mod; b = b % mod; return (((a + b) % mod) + mod) % mod; }
ll mod_subtract(ll a, ll b) { a = a % mod; b = b % mod; return (((a - b + mod) % mod) + mod) % mod; }
ll ceil_divide(ll a, ll b) { return a % b == 0 ? a / b : a / b + 1; }
...
ll binaryToDecimal(string s) { return bitset<64>(s).to_ullong(); }
string decimalToBinary(ll a) { return bitset<64>(a).to_string(); }
ll factorial(ll n) { if (n == 0) { return 1; } ll ans = 1; for (ll i = 1; i <= n; i++) { ans = mod_multiply(ans, i); } return ans; }
ll combination(ll n, ll r) { if (n < r) { return 0; } ll ans = factorial(n); ans = mod_multiply(ans, mod_inverse(factorial(r))); ans = mod_multiply(ans, mod_inverse(factorial(n-r))); return ans; }
