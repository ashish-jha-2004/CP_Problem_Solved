...
void solve(){
    ...
    d_n(length);
    vl arr(length);
    vector<char> big(length);
	for(int index = 0; index < length; index++) cin >> arr[index];
	for(int index = 0; index < length; index++) cin >> big[index];
	for(int index = 0; index < length; index++) pref[index] = (index == 0 ? arr[index] : pref[index - 1] + arr[index]);
	int iterator = length - 1;
	ll result = 0;
	fl(index, length) {
		if(big[index] == 'L') {
			while(iterator > index and big[iterator] != 'R') iterator--;
			if(iterator > index) {
				result += pref[iterator] - (index == 0 ? 0 : pref[index - 1]);
				iterator--;
			}
		}
	}
