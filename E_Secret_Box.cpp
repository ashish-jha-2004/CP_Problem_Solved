...
ll calculateSubGrids(ll rows, ll cols, ll depth, ll i, ll j, ll ans, ll k) {
    ll t1 = (rows - i + 1);
    ll t2 {(cols - j + 1)};
    ll t3 = t1 * t2;
    ll temp = (k / (i * j));
    ll remaining = (depth - temp + 1);
    ans = t3 * remaining;
    return ans;
}

ll countSubGrids(ll rows, ll cols, ll depth, ll k) {
    ll maxi = 0;
    ll ans = 0;
    fn(i, 1, rows+1) {
        fn(j, 1, cols+1) {
            if (k % (i * j) == 0) {
                if ((k / (i * j)) > depth) {
                    ...
                    ans = calculateSubGrids(rows, cols, depth, i, j, ans, k);
                    maxi = max(maxi, ans);
                }
            }
        }
...
void solve(){
    ...
    d_n(depth);
    d_n(k);
    cout << countSubGrids(rows, cols, depth, k) << en;
