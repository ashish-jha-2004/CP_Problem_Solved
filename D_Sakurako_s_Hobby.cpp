...
void solve() {
    ...
    d_s(s);
    s = "1" + s; // Adjust indexing to match v

    vl ans(n + 1, -1);
    vector<bool> vis(n + 1, false); // To keep track of visited nodes

    fn(i, 1, n + 1) {
        if (ans[i] != -1) continue; // Already computed
        ...
        // Traverse the cycle
        while (st.find(temp) == st.end() && !vis[temp]) {
            vis[temp] = true; // Mark node as visited
            if (s[temp] == '0') cb++;
            st.insert(temp);
            temp = v[temp];
        }

        // If the cycle was previously computed, use its result
        if (ans[temp] != -1) {
            cb += ans[temp];
        }

        // Assign the computed value to all nodes in the cycle
        for (auto &node : st) {
            ans[node] = cb;
        }
    }
