...
ll func(char prev = 'd', int ind = 0) {
    if (ind >= n) return 0;
    ll ans = INT_MIN;
    if (dp[prev - 'a'][ind] != -1) return dp[prev - 'a'][ind];
    if (prev == 'a') {
        ans = max(ans, func('b', ind+1) + b[ind]);
        ans = max(ans, func('c', ind+1) + c[ind]);
    }
    else if (prev == 'b') {
        ans = max(ans, func('a', ind+1) + a[ind]);
        ans = max(ans, func('c', ind+1) + c[ind]);
    }
    else if (prev == 'c') {
        ans = max(ans, func('b', ind+1) + b[ind]);
