...
int func(int ind = 0, bool flag = true) {
    if (ind >= n) return 0;
    ...
    if (flag) {
        if (ind + 1 < n)
            ans = ans + min(func(ind + 1, flag ^ 1) + (v[ind] == 1), func(ind + 2, flag ^ 1) + (v[ind] == 1) + (v[ind+1] == 1));
        else ans += func(ind + 1, flag ^ 1) + (v[ind] == 1);
    }
    else {
        if (ind + 1 < n)
            ans = ans + min(func(ind + 1, flag ^ 1), func(ind + 2, flag ^ 1));
        else ans += func(ind + 1, flag ^ 1);
    }
