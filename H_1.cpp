...
vector<vector<int>> graph;   // Global variable for the graph
...
int timeCounter = 0;         // Global time counter for discovery times
...
// Function to perform Tarjan's DFS
void tarjanDFS(int u) {
    disc[u] = low[u] = ++timeCounter;
    st.push(u);
    inStack[u] = true;

    // Go through all the vertices adjacent to this
    for (int v : graph[u]) {
        if (disc[v] == -1) {
            ...
            // If v is in stack, then it's part of the current SCC
...
void findSCCs(int V) {
    ...
    // Call the recursive helper function to find SCCs in DFS tree with root u
    for (int i = 0; i < V; ++i) {
        if (disc[i] == -1) {
            tarjanDFS(i);
        }
...
// Function to check if two nodes are in the same SCC
bool areConnected(int u, int v) {
    for (const auto& scc : sccs) {
        bool foundU = false, foundV = false;
        for (int node : scc) {
            if (node == u) foundU = true;
            if (node == v) foundV = true;
        }
