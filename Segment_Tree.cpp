...
void buildTree (vl &tree, ll array[], ll index, ll low, ll high) {
	if (low == high)
...
ll rangeQuery (vl &tree, ll index, ll low, ll high, ll l, ll r) {
	if (l > r)
		return 0;
	if (l == low && r == high)
		return tree[index];
...
void updateQuery (vl &tree,ll index, ll low, ll high, ll pos, ll delta) {
	if (low == high)
		tree[index] = delta;
	else {
		ll mid = (low + high) >> 1;
