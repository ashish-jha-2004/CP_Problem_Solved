...
void solve(){
    ...
    fn(i, 1, n+1) cin >> c[i], s.make_set(i);
    fn(i, 1, n+1) cin >> k[i];
    vector<pair<ll, pair<int, int>>> edges;
    for (int i = 1; i <= n; ++i) {
        ll wt = c[i];
        edges.push_back(make_pair(wt, make_pair(0, i)));
    }
    fn(i, 1, n+1) {
        fn(j, i+1, n+1) {
            ll dist = abs(position[i].first - position[j].first) + abs(position[i].second - position[j].second);
    ...
    for (auto &lk: connections) {
        cout << lk.first << " " << lk.second << endl;
    }
}
