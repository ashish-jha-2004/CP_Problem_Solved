...
auto cmp = [](const pll &a, const pll &b) {
    return a > b; // Compare pairs in reverse order
};

priority_queue<pll, vector<pll>, decltype(cmp)> initializePriorityQueue(int b, decltype(cmp) comparator) {
    priority_queue<pll, vector<pll>, decltype(cmp)> pq(comparator);
    fl(j, b) {
        auto temp = MP(1, j);
        pq.push(temp); 
    }
...
ll computePriorityQueue(priority_queue<pll, vector<pll>, decltype(cmp)>& pq, ll remaining_capacity, vl& weights, vl& rewards) {
    ll max_reward = 0;
    while (!(remaining_capacity <= 0)) {
        max_reward = pq.top().F; 
        while (!pq.empty() && pq.top().F == max_reward) {
            ll index = pq.top().S;
            pq.pop();
            remaining_capacity -= weights[index];
            pq.push(MP(max_reward + rewards[index], index));
        }
    }
    return max_reward;
}
...
void solve(){
    ...
    // Initialize priority queue by passing the comparator
    auto pq = initializePriorityQueue(b, cmp);

    // Compute answer
    ll ans = computePriorityQueue(pq, a, x, y);
