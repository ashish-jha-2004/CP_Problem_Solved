...
ll func(vl &v, int i, int j, bool flag) {
    if (j == i) {
        if (flag) return dp[i][j][flag] = v[j];
        else return 0;
    }
    if (dp[i][j][flag] != -1) return dp[i][j][flag];
    ll ans = INT64_MIN;
    if (flag) {
        ans = max(ans, max(func(v, i+1, j, 0) + v[i], func(v, i, j-1, 0) + v[j]));
    }
    else {
        ans = max(ans, min(func(v, i+1, j, 1), func(v, i, j-1, 1)));
    }
