...
void pfunc(vector<pair<ll, ll>> &v1, vector<pair<ll, ll>> &v2) {
    for (auto &k: v1) {
        cerr << k.F << " " << k.S << " ";
    }
    cerr << en;
...
void solve(){
    ...
    // pfunc(vab, vba);
    if (vab.empty() or vba.empty()) {
        no;
        return;
    }
    sort(all(vab));
    sort(all(vba));
    if (!(vab[0].F == vba[vba.size() - 1].F or vab[0].S == vba[vba.size() - 1].S or vab[0].S == vba[vba.size() - 1].F or vab[0].F == vba[vba.size() - 1].S)) {
        yes;
        return;
    }
    if (!(vba[0].F == vab[vab.size() - 1].F or vba[0].S == vab[vab.size() - 1].S or vba[0].S == vab[vab.size() - 1].F or vba[0].F == vab[vab.size() - 1].S)) {
        yes;
        return;
    }
    no;
