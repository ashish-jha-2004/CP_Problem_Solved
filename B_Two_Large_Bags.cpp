...
bool canEqualize(int x, int carry) {
    if (x == maj + 1) 
        return (carry == 0);
    if (memo[x][carry] != -1)
        return memo[x][carry];
    ...
    for (int k = 0; k <= Fi; k++) {
        ...
        if (k > 0 && r < 2)
            continue;
        if (k <= mc && canEqualize(x + 1, k))
            return memo[x][carry] = 1;
    }
    return memo[x][carry] = 0;
}
...
void solve(){
    ...
    mc = 2 * n;
    memo.assign(maj + 2, vector<ll>(mc + 1, -1));
    
    canEqualize(1, 0) ? yes : no;
}
