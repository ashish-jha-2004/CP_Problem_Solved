...
bool checkIncreasingOrder(const vl& Ag) {
    int n = Ag.size();
    bool Aj = true;
    fl(i, n - 1) {
        if (Ag[i] > Ag[i + 1]) {
...
void solve(){
    ...
    vl Ag;
    fl(i, num - 1) Ag.PB(gcd(elements[i], elements[i + 1]));
    ...
    vl me1, me2, me3, Amg1, Amg2, Amg3;
    fl(i, num) {
        if (i != index + 1) me1.PB(elements[i]);
        if (i != index + 2) me2.PB(elements[i]);
        if (i != index) me3.PB(elements[i]);
    }

    fl(i, num - 2) {
        Amg1.PB(__gcd(me1[i], me1[i + 1]));
        Amg2.PB(__gcd(me2[i], me2[i + 1]));
        Amg3.PB(__gcd(me3[i], me3[i + 1]));
    }

    bool Aj1 = checkIncreasingOrder(Amg1);
    bool Aj2 = checkIncreasingOrder(Amg2);
    bool Aj3 = checkIncreasingOrder(Amg3);
