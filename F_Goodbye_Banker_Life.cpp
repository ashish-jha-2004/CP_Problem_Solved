...
string func(ll n, string &k) {
    if (n == 1) {
        return k;
    }
    ll max_pwr_2 = (ll)pow(2, ceil(log2(n)));
    ll diff = max_pwr_2 - n;
    ll left_n = (n - diff) / 2;
    ll right_n = left_n;
    string left = func(left_n, k);
    string middle = "";
    while (diff--) middle.push_back('0'), middle.push_back(' ');
    string right = func(right_n, k);
