...
void solve(){
    ...
    if (cnt.size() == 1) {
        vl te = v;
        vl t2 = v;
        reverse(t2.B+cnt[0], t2.E);
        // cerr << cnt.back() << en;
        reverse(te.B, te.B+cnt.back()+1);
        if (check(temp, te)) {
            cout << "yes\n";
            cout << "1 "<< cnt[0]+1 << en;
            return;
    ...
    if (cnt.size() == 2) {
        vl te = v;
        reverse(te.B+cnt[0], te.B+cnt[1]+1);
        if (check(temp, te)) {
            cout << "yes\n";
            cout << cnt[0]+1 << " " << cnt[1]+1 << en;
        }
        else {
            cout << "no\n";
        }
