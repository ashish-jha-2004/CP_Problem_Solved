...
bool check(string recipie, ll nb, ll ns, ll nc, ll pb, ll ps, ll pc, ll r, ll mid) {
    // base case
    if (mid == 0) return true;

    ll cntB = 0, cntS = 0, cntC = 0;
    fl(i, recipie.size()) {
        (recipie[i] == 'B') ? cntB++ : (recipie[i] == 'S') ? cntS++ : cntC++;
    }
    ll reqB = cntB * mid, reqS = cntS * mid, reqC = cntC * mid;
    reqB -= nb;
    if (reqB > 0) r -= (reqB * pb);
    reqS -= ns;
    if (reqS > 0) r -= (reqS * ps);
...
void solve(){
    // code here
    d_s(recipie);
    d_n(nb, ns, nc, pb, ps, pc, r);
    ll low = 0, high = 1e13;
    ll ans = 0;
    while (low <= high) {
        ll mid = (low + high) / 2;
        if (check(recipie, nb, ns, nc, pb, ps, pc, r, mid)) {
            ans = mid;
            low = mid + 1;
        }
        else high = mid - 1;
