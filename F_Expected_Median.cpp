...
ll C(ll n, ll k) {
	if(n < k) return 0LL;
	return (fact[n] * modpow((fact[n - k] * fact[k]) % mod, mod - 2)) % mod;
}
...
void solve(){
    ...
    ll rq1 = k/2 + 1, rq0 = k/2;
    while (rq1 <= cnt1 and rq0 >= 0 and rq0 <= cnt1 and rq1 >= 0) {
        ans = (ans + ((C(cnt1, rq1)%MOD) * (C(cnt0, rq0)%MOD))%MOD) % MOD;
        rq1++;
        rq0--;
    }
    cout << ans << en;
