...
ll now(ll rows, ll cols, vector<vector<bool>> &v) {
    // base case
    if (rows < 0 || cols < 0) return 0;
    if (!v[rows][cols]) return 0;
    if (rows == 0 && cols == 0) return 1;
    if (dp[rows][cols] != -1) return dp[rows][cols];

    // process
    ll ans = 0;
    if (rows > 0) ans = (ans + now(rows - 1, cols, v)) % MOD;
    if (cols > 0) ans = (ans + now(rows, cols - 1, v)) % MOD;
...
void solve(){
    ...
    if (v[0][0] == 0) cout << 0 << en;
    else cout << now(n-1, n-1, v);
    return;
}
