...
ll func(ll &n, ll &k, ll &d, ll sum = 0, bool flag = false){

    if (sum >= n) {
        if (sum == n and flag) return dp[sum][flag] = 1;
        return dp[sum][flag] = 0;
    }
    if (dp[sum][flag] != -1) return dp[sum][flag]%MOD;

    ll ans = 0;
    
    for (ll i = 1; i <= k; i++) {
        ans = mod_add(ans, func(n, k, d, sum + i, flag | (i >= d))); 
    }
