...
void solve(){
	...
	if(N % 2 == 0 || M % 2 == 0){
		...
		if(N == 4 && M == 5){
			good = {{0, 1}, {2, 0}, {3, 2}, {2, 4}};
		} else if(N == 5 && M == 4){
			good = {{1, 0}, {0, 2}, {2, 3}, {4, 2}};
		}
		int cur = 1;
		for(auto [x, y] : good){
			grid[x][y] = cur;
		...
		int z = N*M + 1 - good.size() * 3;
		for(auto [x, y] : good){
			vector<pair<int,int> > neighbors;
			for(int dx = -1; dx <= 1; dx++){
				for(int dy = -1; dy <= 1; dy++){
					if(abs(dx) + abs(dy) <= 1 && abs(dx) + abs(dy) > 0){
						neighbors.push_back({x + dx, y + dy});
					}
				}
			}
			unsafe[x][y] = -1;
			for(auto [c, d] : neighbors){
	...
	vector<vector<int> > played(N, vector<int>(M, 0));
	vector<vector<int> > allowed(N, vector<int>(M, 0));
	for(int i = 0; i < N*M; i++){
		int x, y;
		if(i % 2 == 0){
			cin >> x >> y;
			...
			for(int a = 0; a < N; a++){
				for(int b = 0; b < M; b++){
					if(allowed[a][b] == 1 && played[a][b] == 0){
						int score = grid[a][b];
						if(unsafe[a][b] == 1) continue;
						if(best == make_pair(-1, -1) || score < grid[best.first][best.second]){
							best = {a, b};
						}
					}
				}
