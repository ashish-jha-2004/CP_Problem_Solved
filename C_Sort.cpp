...
ll is_p(vector<vector<ll>> &vm1, vector<vector<ll>> &vm2, ll l, ll r) {
    auto ml1 = vm1[l];
    auto ml2 = vm2[l];
    auto mr1 = vm1[r];
    auto mr2 = vm2[r];
    ll cnt = 0;
    for (char i = 'a'; i<='z'; i++) {
        if (mr1[i-'a'] - ml1[i-'a'] != mr2[i-'a'] - ml2[i-'a']) cnt += abs((mr1[i-'a'] - ml1[i-'a']) - (mr2[i-'a'] - ml2[i-'a']));
    }
    return cnt;
}
