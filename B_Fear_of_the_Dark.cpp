...
bool check(ld a, ld b, ld c, ld d, ld e){
    return e >= sqrt(pow(a-c, 2)+pow(b-d, 2));
}
...
void solve(){
    ...
    ld w = dist(ax, ay, bx, by)/2;
    ld temp = max(dist(ax, ay, 0, 0), dist(ax, ay, px, py));
    ld temp1 = max(dist(bx, by, 0, 0), dist(bx, by, px, py));
    ld both = max ({min(dist(ax, ay, px, py), dist(bx, by, px, py)), min(dist(ax, ay, 0, 0), dist(bx, by, 0, 0)), w});
    cout << fixed << setprecision(10) << min({temp, temp1, both}) << en;
    return;
}
