...
const int Weight = 1e5+10;
ll N, W;
int weight[Weight], value[Weight];
ll dp[100][Weight];

ll func(int ind = 0, int sum = 0) {
    if (ind >= N) return 0;
    if (dp[ind][sum] != -1) return dp[ind][sum];

    ll ans = INT_MIN;
    if (sum + weight[ind] <= W) {
        ans = max({ans, func(ind+1, sum), func(ind+1, sum+weight[ind]) + value[ind]});
    }
    else {
        ans = max({ans, func(ind+1, sum)});
    }
