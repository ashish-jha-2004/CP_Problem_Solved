...
bool check(multiset<ll> &ms1, ll &y){
    if (ms1.empty()) return 0;
    if (*(ms1.B) <= y) {
        return 1;
    }
    return 0;
...
bool check1(multiset<ll> &ms2, ll &y1){
    if (ms2.lower_bound(y1) != ms2.E) {
        return true;
    }
    return false;
}
...
bool cmp(const pair<int, int> &a, const pair<int, int> &b) {
    if (a.first != b.first) {
        ...
        return a.second < b.second; // If first elements are the same, sort by second element in increasing order
    }
}
...
bool cmp1(const pair<int, int> &a, const pair<int, int> &b) {
    if (a.first != b.first) {
        ...
        return a.second > b.second; // If first elements are the same, sort by second element in decreasing order
    }
}
