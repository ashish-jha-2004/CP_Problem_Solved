...
bool can_put(ll &pref,ll &suf, string &s, int ind, ll k) {
    if (ind - pref > k and suf - ind > k) return true;
    return false;
}
...
void solve(){
    ...
    cin >> n >> k;
    d_s(s);
    ll pref = INT_MIN, suf = INT_MAX;
    int sf = 1;
    while (sf < n and s[sf] != '1') sf++;
    if (sf == n) sf = INT_MAX;
    suf = sf;
    ll cnt = 0;
    fl(i, n) {
        if (s[i] == '1') {
            ...
            // debug(pref, suf);
            cnt += can_put(pref, suf, s, i, k);
            if (can_put(pref, suf, s, i, k)) s[i] = 'i', pref = i;
            // debug(cnt);
        }
    }
    cout << cnt << en;
