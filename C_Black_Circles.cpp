...
ld distance(ld x1, ld x2, ld x3, ld x4) {
    return (x1 - x3) * (x1 - x3) + (x2 - x4) * (x2 - x4);
}

bool check(vector<pair<ld, ld>> &vp, ld mini) {
    fl(i, vp.size()) {
        ld dt = distance(vp[i].F, vp[i].S, xt, yt);
        if (dt <= mini) {
            return false;
        }
    }
...
void solve(){
    ...
    cin >> xs >> ys >> xt >> yt;
    ld mini = distance(xs, ys, xt, yt);
    check(vp, mini) ? yes : no;
}
