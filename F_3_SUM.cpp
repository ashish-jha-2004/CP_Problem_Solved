...
bool check (multiset<ll> &ms) {
    fl(i, vt.size()) {
        ll a = vt[i][0], b = vt[i][1], c = vt[i][2];
        auto it1 = ms.find(a);
        ll a1 = -1, b1 = -1, c1 = -1;
        if (it1 != ms.E) a1 = *it1, ms.erase(it1);
        auto it2 = ms.find(b);
        if (it2 != ms.E) b1 = *it2, ms.erase(it2);
        auto it3 = ms.find(c);
        if (it3 != ms.E) c1 = *it3, ms.erase(it3);
        if (it1 != ms.E and it2 != ms.E and it3 != ms.E) {
            return true;
        }
        if (a1 != -1) ms.insert(a1);
        if (b1 != -1) ms.insert(b1);
        if (c1 != -1) ms.insert(c1);
    }
    return false;
}
