...
void dfs(int node, unordered_map<ll, ll> &graph, vector<int> &v2) {
    vis[node] = true;
    v2.push_back(node);
    cnt++;
    if (graph[node] == -1 or graph[node] == 0) return;
    dfs(graph[node], graph, v2);
}
...
void solve(){
    ...
    d_n(n);
    d_v(a, n);
    // unordered_map<ll, ll> graph;
    // fl(i, n) {
    //     if (graph.find(v[i] - 1) == graph.end()) {
    //         graph[v[i]] = -1;
    //     }
    //     else {
    //         graph[v[i] - 1] = v[i];
