...
void solve(){
    ...
    for (int i{0}; i<n; i++) {
        for (int j{0}; j<m; j++) {
            ...
            if (v[i][j] == 'U') {
                if (cnt - m < 0) {
                    graph[cnt].push_back(0);
                }
                else {
                    graph[cnt].push_back(cnt - m);
                }
            }
            else if (v[i][j] == 'D') {
                if (cnt + m > n) {
    ...
    fre.insert(0);
    while (cut < n * m) {
        if (vis[cut] == false and frame[cut] != '?') {
            bool ng = true;
            dfs(cut, ng);
            if (ng == false) {
                debug(cut, graph[cut]);
    ...
    fl(i, n) {
        fl(j, m) {
            if (v[i][j] == '?') {
                bool nr = true;
                if (i - 1 >= 0) {
                    if (fre.count(vp[i-1][j]) == 0 and nr) ans++, nr = !nr;
                }
                if (i + 1 < n) {
                    if (fre.count(vp[i+1][j]) == 0 and nr) ans++, nr = !nr;
                }
                if (j - 1 >= 0) {
                    if (fre.count(vp[i][j-1]) == 0 and nr) ans++, nr = !nr;
                }
                if (j + 1 < m) {
                    if (fre.count(vp[i][j+1]) == 0 and nr) ans++, nr = !nr;
                }
