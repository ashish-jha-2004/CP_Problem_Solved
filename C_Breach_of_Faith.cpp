...
void solve(){
    ...
    // case 1 first n + 1 elements are still there
    ll sum_1 = 0, sum = 0;
    fl(i, 2 * n) {
        if (i < n + 1) sum_1 += v[i];
        sum += v[i];
    ...
    ll diff = sum - sum_1;
    if (st.count((ll)abs(diff - sum_1)) == 0 and sum_1 - diff > 0) {
        v.push_back((ll)abs(diff - sum_1));
        vl left, right;
        fl(i, 2 * n + 1) {
            if (i <= n) left.push_back(v[i]);
    ...
    if (n == 1) {
        cout << max(v.front(), v.back()) << " " << v.front() + v.back() << " " <<  min(v.front(), v.back()) << en;
        return;
    }
    sort(all(v));
    ...
    debug(sum_odd, sum_even);
    v.push_back((ll)abs(sum_odd - sum_even));
    cout << v << en;
