...
ll func(ll ind, ll prev, vector<pair<ll, ll>> &vp) {
    // base case
    if (ind >= vp.size() - 1) {
        return 0;
    }
    if (mp.find({ind, prev}) != mp.E) return mp[{ind, prev}]; 

    // transition
    ll ans = 0;
    if (vp[ind].F - vp[ind].S > prev and vp[ind].S < vp[ind+1].F - vp[ind].F) {
        ans = max(func(ind+1, vp[ind].F, vp) + 1, func(ind+1, vp[ind].F + vp[ind].S, vp) + 1);
    }
    else if (vp[ind].F - vp[ind].S > prev) {
        ans = func(ind+1, vp[ind].F, vp) + 1;
    }
    else if (vp[ind].S < vp[ind+1].F - vp[ind].F) {
        ans = func(ind+1, vp[ind].F + vp[ind].S, vp) + 1;
    }
    else {
        ans = func(ind+1, vp[ind].F, vp);
    }
    return mp[{ind, prev}] = ans;
}
