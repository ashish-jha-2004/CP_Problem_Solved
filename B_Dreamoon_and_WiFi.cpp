...
bool is_same1(vector<short> &v, ll cnt2) {
    ll cnt = 0;
    fl(i, v.size()) {
        if (v[i] == 1) cnt++;
    }
    return (cnt2 == cnt) ? true : false;
}

vector<vector<short>> vs;

void func(vector<short> v, ll ind) {
    if (ind < 0) {
        vs.PB(v);
        // cerr << "hii";
        return;
...
void solve(){
    ...
    if (!f1) {
        ...
        // cerr << cnt1 << en;
        cout << fixed << setprecision(12) << cnt1 / total ;
    }
}
