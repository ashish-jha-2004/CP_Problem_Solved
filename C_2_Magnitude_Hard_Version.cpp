...
ll func1(vl &v, int ind = 0, ll sum = 0) {
    if (ind >= v.size()) return sum;
    return max(func1(v, ind+1, sum + v[ind]), func1(v, ind+1, abs(sum + v[ind])));
}

ll func2(vl &v, ll maxi, ll ind = 0, ll sum = 0) {
    if (ind >= v.size()) return sum == maxi;
    return func2(v, maxi, ind+1, sum + v[ind]) + func2(v, maxi, ind+1, abs(sum + v[ind]));
}
...
void solve(){
    ...
    ll prev = 0;
    fl(i, n) {
        if (mini < 0 and pref[i] == mini) vp.PB({pref[i], i});
        cnt[i] = prev + (pref[i] < 0), prev = cnt[i]; 
    }
    if (vp.size() == 0) {
        cout << modpow(2, n) << en;
        return;
    ...
    ll ans = 0;
    for (auto &k: vp) {
        ans = (ans + modpow(2, n - cnt[k.S]))%MOD;
    }
    cout << ans << en;
