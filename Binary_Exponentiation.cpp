...
ll binary_exponentiation(ll a, ll b){
    ...
    // case 1
    if (b&1) return a * binary_exponentiation(a, (b-1)/2) * binary_exponentiation(a, (b-1)/2);

    // case 2
    else return binary_exponentiation(a, b/2) * binary_exponentiation(a, b/2);
}

// Itertive method -->
ll binary_exponentiation_iterative(ll a, ll b) {
    ll ans = 1;
    while (b) {
        if (b & 1){
            ans *= a;
...
void solve(){
    ...
    d_n(n);
    d_n(x);
    cout << binary_exponentiation(n, x) << " " << binary_exponentiation_iterative(n, x);
}
