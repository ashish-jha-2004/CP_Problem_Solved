...
unordered_map<ll, bool> visited;  // Use map to handle large values
ll n;

void dfs(ll node, vector<ll> &result) {
    visited[node] = true;
...
void solve() {
    // Clear the graph and visited map for every test case
    graph.clear();
    visited.clear();
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (x % 3 == 0 && s.find(x / 3) != s.end()) {
            graph[x].push_back(x / 3);
        }
        if (s.find(x * 2) != s.end()) {
            graph[x].push_back(x * 2);
        }
    }

    // DFS and finding the valid sequence
    for (int i = 0; i < n; i++) {
        visited.clear();  
        vector<ll> vec;
        dfs(v[i], vec);
