...
void solve(){
    ...
    ll mini = INT_MAX;
    for (int i{0}; i<n; i++) {
        // maxi = max(maxi, (pref1[i+1] - pref1[0]) + (pref2[n] - pref2[i]));
        // cerr << en;
        debug(i);
        debug(mini, pref2[i], pref2[0], pref1[n] - pref2[i+1]);
        mini = min(mini, max(pref2[i] - pref2[0], pref1[n] - pref1[i+1]));
    }
    cout << mini << en;
}
