...
const int N = 2e5;
set<pair<int, pair<int, int>>> compute;
...
void solve(){
    ...
    set<pair<int, pair<int, int>>> dis_vis;
    auto it = compute.B;
    ll ne = v.size();
    while (ne--) {
        dis_vis.insert(*it);
    ...
    for (auto &it1: dis_vis) {
        auto it = it1.S;
        auto a1 = it, a2 = it, a3 = it, a4 = it;
        a2.S++, a3.F++, a4.F++, a4.S++;
        state.insert({a1.F + a1.S, {a1.F, a1.S}});
        state.insert({a2.F + a2.S, {a2.F, a2.S}});
        state.insert({a3.F + a3.S, {a3.F, a3.S}});
        state.insert({a4.F + a4.S + 2, {a4.F, a4.S}});
    }
    vector<pair<int, int>> ans;
    fl(i, n) {
        if (v[i] == 1) {
            auto st = *state.B;
            ans.push_back({st.S.F, st.S.S});
            state.erase(state.B);
            if (dis_vis.count(st)) dis_vis.erase(st);
        }
        else {
            auto st = *dis_vis.B;
            ans.push_back({st.S.F, st.S.S});
            state.erase(st);
            dis_vis.erase(dis_vis.B);
        }
    }
    for (auto it: ans) {
        cout << it.F << " " << it.S << en;
...
int main(){
    ...
    for (int y{1};; y += 3) {
        pair<int, int> it = {1, y};
        while (it.S >= 1) {
            compute.insert({it.F + it.S, {it.F, it.S}});
            it.F += 3;
            it.S -= 3;
        }
        if (compute.size() >= N) break;
    }
