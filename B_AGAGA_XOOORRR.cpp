...
void solve() {
    ...
    // Case 1: Overall XOR is zero.
    if (total == 0) {
        cout << "YES" << "\n";
        return;
    }
    
    // Case 2: Look for at least two indices where the prefix XOR equals total.
    int cnt = 0;
    int prefix = 0;
    // We stop at n-1 because we need the third segment to be non-empty.
    for (int i = 0; i < n - 1; i++) {
        prefix ^= a[i];
        if (prefix == total) {
            cnt++;
            // Reset prefix so that we start counting a new segment.
            prefix = 0;
        }
    }
