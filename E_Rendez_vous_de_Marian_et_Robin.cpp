...
void dijkstra(ll n, ll src, vector<ll> &time, vector<vector<pair<ll, ll>>> &adj)
{
    time[src] = 0;
    ...
    while (!pq.empty())
    {
        ...
        for (auto child : adj[node])
        {
            ...
            ll childTime = child.second;
 
            if (time[childNode] > (childTime + curTime))
            {
                time[childNode] = (childTime + curTime);
                pq.push({time[childNode], childNode});
            }
        }
    }
}
...
void solve() {
    ...
    vector<ll> dist1(2 * n + 1, 1e18);
    vector<ll> dist2(2 * n + 1, 1e18);
    ...
    dijkstra(2 * n, 1, dist1, g);
    dijkstra(2 * n, n, dist2, g);
