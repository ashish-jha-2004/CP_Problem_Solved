...
ll func(vl &v, ll ind = 0, ll sum = 0) {
    if (sum > sm/2) return 0;
    if (ind >= v.size()) {
        if (sum == sm / 2) return 1;
        else return 0;
    }
    if (dp[ind][sum] != -1) return dp[ind][sum];
    return dp[ind][sum] = (func(v, ind+1, sum+v[ind]) % MOD + func(v, ind+1, sum) % MOD) % MOD;
}
