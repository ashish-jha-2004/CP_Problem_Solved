...
bool func(vl &v, ll i, ll sum, ll s1 = 0) {
    // base case
    if (i < 0 and sum != s1) return false;
    if (i < 0) return true;
    if (dp[i][s1] != -1) return dp[i][s1];
    // process
    bool take = func(v, i-1, sum, s1+v[i]);
    bool n_take = func(v, i-1, sum, s1);

    return dp[i][s1] = take || n_take;
}
