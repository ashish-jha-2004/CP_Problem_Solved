...
int maxBooks(vector<ll>& books, int t) {
    int n = books.size();
    ...
    // For each starting book i, find the farthest book j that Valera can read
    for (int i = 0; i < n; ++i) {
        int left = i, right = n - 1;
        while (left <= right) {
            int mid = left + (right - left) / 2;
            int totalTime = prefixSum[mid] - (i > 0 ? prefixSum[i - 1] : 0);
            if (totalTime <= t) {
                maxBooksRead = max(maxBooksRead, mid - i + 1);
                left = mid + 1;
            } else {
                right = mid - 1;
            }
