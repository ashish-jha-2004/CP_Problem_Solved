...
bool func(vector<string> &vs) {
    // upper and lower check
    fl(i, vs[0].size()) {
        if (vs[0][i] != '1') {
            return false;
        }
        if (vs[vs.size()-1][i] != '1') return false;
    }

    // generally
    for (int i{1}; i<vs.size()-1; i++) {
        if (vs[i][0] != '1') return false;
        if (vs[i][vs.size()-1] != '1') return false;
        for (int j{1}; j<vs.size()-1; j++) {
            if (vs[i][j] != '0') return false;
        }
    }
    return true;
}
...
void solve(){
    ...
    for (int i{0}; i<n-ti+1; i+=ti) {
        string temp;
        for (int j{i}; j<i+ti; j++) {
            temp.PB(s[j]);
        }
        vs.PB(temp);
    }
