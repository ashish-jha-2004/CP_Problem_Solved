...
ll cnt_y(ll l2, ll r2, ll k, ll x) {
    ll pw = 1, cnt = 0;
    while (x * pw <= r2) {
        if  (x * pw >= l2) {
            cnt++;
...
ll maxi_pnt(ll l2, ll r2, ll k, ll x, ll r1) {
    ll maxi = cnt_y(l2, r2, k, x);
    bool flag = false;
    int pw = 2;
    ll min_x = x;
    while (pw * x <= r1) {
        if  (cnt_y(l2, r2, k, x * pw) != maxi) {
            flag = true;
            break;
        }
        x *= pw;
...
void solve(){
    // code here
    d_n(k, l1, r1, l2, r2);
    auto maxi = cnt_y(l2, r2, k, 11);
    auto mini = cnt_y(l2, r2, k, r1);
    // debug(sp);
    ll ans = 0, cnt = maxi;
    if (maxi == mini) {
        cout << (r1 - l1 + 1) * maxi << en;
        return;
    }
    while (cnt <= mini) {
