...
bool is_div8(string &temp) {
    if (temp.empty()) return false; // Handle empty string
    ll kk;
    if (temp.size() >= 3) {
        string t;
        t.PB(temp[temp.size()-3]);
        t.PB(temp[temp.size()-2]);
        t.PB(temp[temp.size()-1]);
        kk = stoi(t);
    }
    else {
        kk = stoi(temp);
...
bool func(string &s, short ind, string temp = "") {
    if (ind >= s.size()) {
    ...
    if (dp[ind][rd(temp)] != -1) return dp[ind][rd(temp)];

    return dp[ind][rd(temp)] = (func(s, ind+1, temp + s[ind])) || (func(s, ind+1, temp));
