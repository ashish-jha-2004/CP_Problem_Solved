...
bool check(map<char, set<string>> &mp, char &trump_card, set<char> &s) {
    for (auto &k: s) {
        if (k != trump_card and mp[k].size() >= 2) {
            return false;
        }
    }
    return true;
...
bool check1(map<char, set<string>> &mp, char &trump_card, set<char> &s) {
    for (auto &k: s) {
        if (k != trump_card and mp[k].size() % 2 == 1) {
            return true;
        }
    }
    return false;
...
void solve(){
    ...
    s.insert('D');
    s.insert('H');
    if (2*mp[trump_card].size() == (mp['C'].size() + mp['S'].size() + mp['D'].size() + mp['H'].size())) {
        for (auto &l: s) {
            if (!mp[l].empty() and !mp[trump_card].empty() and l != trump_card) {
                for (auto &k: mp[l]) {
                    cout << k << " " << *mp[trump_card].B << en;
                    mp[trump_card].erase(mp[trump_card].B);
                }
            }
        }
        return;
    ...
    while (true) {
        ...
        if (check(mp, trump_card, s)) {
            break;
        }
    }
    if (2*mp[trump_card].size() >= (mp['C'].size() + mp['S'].size() + mp['D'].size() + mp['H'].size())) {
        for (auto &k: s) {
            if (k != trump_card and !mp[k].empty() and !mp[trump_card].empty()) {
                ans.PB({*mp[k].B, *mp[trump_card].B});
                mp[trump_card].erase(mp[trump_card].B);
                mp[k].erase(mp[k].B);
            }
        }
    ...
    // }
    // cerr << en;
    if (mp[trump_card].size() % 2 == 1){
        cout << "IMPOSSIBLE\n";
        return;
    }
    if (check1(mp, trump_card, s)) {
        cout << "IMPOSSIBLE\n";
        return;
    }
    // cerr << ans.size() << en;
    // cerr << mp[trump_card].size() << en;
    if (!mp[trump_card].empty()) {
        ll jk = mp[trump_card].size()/2;
        fl(i, jk) {
            if (!mp[trump_card].empty()){
                string s1 = *mp[trump_card].B;
                mp[trump_card].erase(mp[trump_card].B);
                string s2 = *mp[trump_card].B;
                mp[trump_card].erase(mp[trump_card].B);
                ans.PB({s1, s2});
            }
        }
