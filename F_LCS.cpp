...
std::string lcsRecursive(const std::string& X, const std::string& Y, int m, int n) {
    ...
    if (X[m - 1] == Y[n - 1]) {
        // Include this character in the LCS and recurse for the remaining substrings
        return dp[m][n] = lcsRecursive(X, Y, m - 1, n - 1) + X[m - 1];
    }
    // If the last characters of both substrings do not match
    else {
        // Recur with either ignoring the last character of X or Y, and choose the longer result
        std::string lcs1 = lcsRecursive(X, Y, m - 1, n);
        std::string lcs2 = lcsRecursive(X, Y, m, n - 1);
        return dp[m][n] = (lcs1.length() > lcs2.length()) ? lcs1 : lcs2;
    }
}
void solve(){
    // code here
