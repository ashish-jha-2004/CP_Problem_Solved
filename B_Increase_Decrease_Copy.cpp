...
ll fmin(vl &a, vl &b) {
    ll sel = b.back();
    ll n = a.size();
    // aisa element jo b.back() se chota ya equal ho and b[that_ind] >= b.back()
    fl (i, n) {
        if (a[i] <= sel and b[i] >= sel) {
            return i;
        }
        if (a[i] >= sel and b[i] <= sel) {
            return i;
...
ll fmin1(vl &a, vl &b) {
    ...
    ll min_diff = INT_MAX;
    fl(i, n) {
        min_diff = min(abs(a[i] - sel), min_diff);
        min_diff = min(min_diff, abs(b[i] - sel));
    }
    ll ind = -1;
    fl (i, n) {
        if (abs(a[i] - sel) == min_diff) {
            ind = a[i];
            break;
        }
        if (abs(b[i] - sel) == min_diff) {
            ind = b[i];
            break;
        }
    }
